/* global React */
// ─────────────────────────────────────────────────────────────
// Thumbnails — abstract CSS/SVG compositions for each card.
// Each component fills its parent .thumb square.
// Palette refs: --moss, --sage, --coral, --butter, --ink, --surface
// ─────────────────────────────────────────────────────────────
...
// 1. Web Design — browser window with abstract page blocks
function ThumbWebDesign() {
  return (
    <div style={{ ...thumbWrap, padding: 10, background: '#F6EED7', flexDirection: 'column', alignItems: 'stretch', gap: 6 }}>
...
// 2. things I've coded — terminal-style ascii
function ThumbCode() {
  return (
    <div style={{ ...thumbWrap, background: '#1b1f1a', padding: 10, flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'flex-start', fontFamily: 'JetBrains Mono, monospace', fontSize: 7.5, color: '#C5D4A8', lineHeight: 1.4 }}>
      <div style={{ color: 'rgba(197,212,168,0.5)' }}>~ patrick/garden $</div>
      <div><span style={{ color: '#F0A488' }}>const</span> {'grow = ()'} {'=>'} {'{'}</div>
      <div>&nbsp;&nbsp;<span style={{ color: '#F7DFA0' }}>plant</span>(seed)</div>
      <div>&nbsp;&nbsp;<span style={{ color: '#F7DFA0' }}>water</span>()</div>
      <div>{'}'}</div>
...
// 3. Side-quests — cork-board pinned notes collage (warm cork background)
function ThumbSideQuests() {
  return (
    <div style={{ ...thumbWrap, background: '#B88A5A', padding: 0 }}>
      {/* cork grain */}
      <div style={{
        position: 'absolute', inset: 0,
        backgroundImage:
      ...
      {/* pinned note 1 — coral square, tilted */}
      <div style={{
        position: 'absolute', left: '12%', top: '16%',
        width: '38%', height: '40%',
        background: 'var(--coral)',
        transform: 'rotate(-6deg)',
        boxShadow: '0 3px 6px rgba(40,20,0,0.25)',
      }}>
      ...
      {/* pinned note 2 — butter rect, tilted other way */}
      <div style={{
      ...
      {/* pinned polaroid — sage */}
      <div style={{
        position: 'absolute', left: '22%', bottom: '10%',
        width: '44%', height: '38%',
        background: 'var(--surface)',
        transform: 'rotate(3deg)',
        boxShadow: '0 3px 6px rgba(40,20,0,0.3)',
        padding: 3,
      }}>
...
// 4. old art blog — vintage photo (sepia-toned abstract)
function ThumbArtBlog() {
  return (
    <div style={{ ...thumbWrap, background: 'linear-gradient(180deg, #D4B88A 0%, #A88B5C 60%, #6B5433 100%)' }}>
      {/* grain + scan lines */}
      <div style={{
        position: 'absolute', inset: 0,
      ...
      {/* subject suggestion: distant figure / tree */}
      ...
      <div style={{ position: 'absolute', bottom: 6, left: 8, fontFamily: 'JetBrains Mono, monospace', fontSize: 7, color: 'rgba(255,240,210,0.8)' }}>MMIX</div>
    </div>
  );
}

// 5. things I wrote — open notebook on a warm desk surface
function ThumbWriting() {
  return (
    <div style={{ ...thumbWrap, background: '#7a9a5c', padding: 0 }}>
      {/* desk grain */}
      <div style={{
        position:'absolute', inset:0,
      ...
      <div style={{
        ...
        background: '#FBF5E8',
        boxShadow: '0 4px 10px rgba(20,20,0,0.25)',
        transform: 'rotate(-3deg)',
        display: 'flex',
      }}>
        {/* spine */}
        ...
        <div style={{ flex: 1, padding: 6, display:'flex', flexDirection:'column', gap:3 }}>
          <div style={{ fontFamily:'Fraunces,serif', fontStyle:'italic', fontSize:8, color:'var(--ink)' }}>on craft</div>
...
// 6. music I make — album art placeholder, moss+coral composition
function ThumbMusic() {
  return (
    <div style={{ ...thumbWrap, background: 'var(--moss)' }}>
...
// 8. Music Video — filmstrip/still
function ThumbMusicVideo() {
  return (
    <div style={{ ...thumbWrap, background: 'linear-gradient(160deg, #2a3b20 0%, #4F6B28 60%, #7a8c4e 100%)' }}>
      {/* moon */}
      ...
      {/* film sprocket holes on left edge */}
...
function ThumbVisuals() {
  return (
    <div style={{ ...thumbWrap, background: 'var(--butter)' }}>
      <svg viewBox="0 0 100 100" style={{ width: '100%', height: '100%' }}>
        ...
        <text x="50" y="95" textAnchor="middle"
          fontFamily="Fraunces, serif" fontStyle="italic" fontSize="8" fill="var(--ink)">
          form / feeling
        </text>
      </svg>
    </div>
...
// 10. UX Design — phone mockup with sticky notes annotations
function ThumbUX() {
  return (
    <div style={{ ...thumbWrap, background: '#3a4a6e', padding: 0 }}>
      {/* subtle grid */}
      <div style={{
        position:'absolute', inset:0,
...
// Map cards to thumbnail components
const THUMBS = {
  web: ThumbWebDesign,
  code: ThumbCode,
